...
$newFile = "$($file.basename)_fixed.docx"

# check if a file with the same name as the fixed one already exists
if (test-path $newFile) {
    write-host "A file named '$newFile' already exists, aborting"
    exit(1)
}
...
$emptyFlags = @(0x00, 0x00)
if (!(arraysMatch $buffer $emptyFlags 6)) {
    write-host "This file has some flags set, needs to be recompressed"
    $needsRecompression = $true
}

# check if the file needs to be recompressed
if ($needsRecompression) {
    $tempFile = "$($file.basename).zip"
    $tempFolder = "./$($file.basename)_temp"
    
    # create a file with the .zip extension (expand-archive doesn't work otherwise)
    copy-item $file $tempFile
    # decompress the file
    expand-archive $tempFile $tempFolder
    # compress it again
    compress-archive "$tempFolder/*" $tempFile -force
    # rename the new file
    move-item $tempFile $newFile
